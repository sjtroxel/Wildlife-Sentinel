...
const GDACS_DR_URL = 'https://www.gdacs.org/gdacsapi/api/events/geteventlist/DR';
...
export class GdacsDroughtScout extends BaseScout {
  constructor() {
    super({
      name: 'gdacs_drought',
      dedupTtlSeconds: 12 * 3_600, // 12h — GDACS updates active drought episodes periodically
      maxConsecutiveFailures: 5,
      circuitOpenMinutes: 60,      // longer reset — droughts update infrequently
    });
  }

  protected async fetchEvents(): Promise<RawDisasterEvent[]> {
    const res = await fetchWithRetry(GDACS_DR_URL);
    ...
    for (const feature of features) {
      ...
      // API should only return DR, but guard defensively
      if (props.eventtype !== 'DR') continue;
      ...
      // episodeid increments with each update to an active drought event
      const eventId = `gdacs_dr_${props.eventid}_ep${props.episodeid}`;
